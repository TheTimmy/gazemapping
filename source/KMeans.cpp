...
KMeans::KMeans(int clusters, int iterations, int batchSize, size_t max_no_improvement, double tolerance) 
...
void KMeans::initClusters(cv::Mat& desc) {
...
void KMeans::fit(cv::Mat& desc) {
    ...
    while (!converged()) {
        ...
        for(size_t i = 0; i < labels.size(); ++i) {
            ...
            N[clusterIndex]++;
            centers.row(clusterIndex) += desc.row(i);
        }
...
void KMeans::label(const cv::Mat& desc, cv::Mat& labels) {
    thread_pool& pool = thread_pool::instance();
...
void KMeans::label(const cv::Mat& desc, std::vector<uint8_t>& labels) {
    thread_pool& pool = thread_pool::instance();
...
void KMeans::distance(const cv::Mat& desc, std::vector<double>& distances) {
    thread_pool& pool = thread_pool::instance();
